...
func (pp *PostProcessing) VizEtf(flowField, dst *gocv.Mat) {
	...
	noise := gocv.NewMatWithSize(flowField.Cols()/2, flowField.Rows()/2, gocv.MatTypeCV32F+gocv.MatChannels3)
	gocv.Randu(&noise, 0.0, 1.0)
	gocv.Resize(noise, &noise, image.Point{flowField.Rows(), flowField.Cols()}, 0, 0, gocv.InterpolationNearestNeighbor)
